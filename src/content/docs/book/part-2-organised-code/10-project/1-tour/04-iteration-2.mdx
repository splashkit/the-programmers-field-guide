...
cheese_data read_cheese()
{
    ...
    result.price_per_kg = read_int("Enter price per kg (cents): ");
...
main_menu_option read_main_menu_option()
{
    ...
    return (main_menu_option)read_int("Select an option (0-2): ", 0, 2);
}
...
void edit_cheese(cheese_data &cheese)
{
    ...
    if ( read_int("Do you want to edit the name? (1 for yes, 0 for no): ", 0, 1) == 1 )
    {
        cheese.name = read_string("Enter new cheese name: ");
    }
    if ( read_int("Do you want to edit the weight? (1 for yes, 0 for no): ", 0, 1) == 1 )
    {
        cheese.weight = read_double("Enter new weight in stock (kg): ");
    }
    if ( read_int("Do you want to edit the price? (1 for yes, 0 for no): ", 0, 1) == 1 )
    {
        cheese.price_per_kg = read_int("Enter new price per kg (cents): ");
    }
}
...
int select_cheese(vector<cheese_data> &cheeses)
{
    ...
    return read_int("Select cheese (0 - " + to_string(cheeses.size()) + "): ", 0, cheeses.size()) - 1;
}
...
main_menu_option read_main_menu_option()
{
    ...
    return (main_menu_option)read_int("Select an option (0-3): ", 0, 3);
}
...
:::tip
If you are using the `vector<cheese_data>`, you can remove an element at an index using: `shop.cheeses.erase(shop.cheeses.begin() + index);`. This is not the most intuitive library, but you can easily find this with a quick search or prompt to a generative AI.
:::
