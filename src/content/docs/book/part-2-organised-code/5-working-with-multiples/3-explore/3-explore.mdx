...
- database - full database... one entity... with a few edit options
- music player - database with music and playing the music (songs)
- game - rock dodge - images/circles/simple player (move, left right, rocks go top bottom)
...
- Add a new randomly positioned NPC to the game at random intervals (a few each second) - (extension) add NPCs when two NPCs are within a relatively small distance of each other.
- Include a "time to live" value in `NPC`. This value:
  - starts at 100
  - gets reduced by one each game loop
  - gets reset to 100 whenever the player character is close
- Check every game loop whether any NPCs have died (i.e., the NPC's "time to live" is equal to or less than 0). (extension) If so, the dead NPC farthest away from the player character should be removed from the game.
