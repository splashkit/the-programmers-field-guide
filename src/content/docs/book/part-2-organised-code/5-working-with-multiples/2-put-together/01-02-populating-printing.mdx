...
3. Copy in your *utilities.h* and *utilities.cpp* files from the the [shared utilities](/book/part-2-organised-code/2-organising-code/2-put-together/04-00-utilities) activity. This will contain the `read_integer` and `read_double` functions we need. If either of these are missing, you should code them in your utilities files.
...
Have a go at coding this. Replace the `//todo: add struct here` comment with the code needed to declare the struct. You should recall how to do this from the [organised data](/book/part-2-organised-code/3-structuring-data/1-concepts/03-01-structs) chapter. In this case, you will need to include the array declaration within the struct. Look back at the [array declaration](/book/part-2-organised-code/5-working-with-multiples/1-concepts/00-01-array-declaration) section if you need. If you get stuck you can check the solution below.
...
1. The [for loop](/book/part-1-instructions/3-control-flow/1-concepts/04-3-for-loop) is used to repeat the loop once for each element in the array.
