...
An array is a special kind of **variable**, one that stores multiple values instead of a single value. The values within an array, called elements, all have the same [type](/book/part-1-instructions/1-sequence/2-trailside/06-type).
...
Arrays store multiple values, with an index that provides access to the individual elements within the array. Conceptually this can be viewed as a [variable](/book/part-1-instructions/1-sequence/2-trailside/07-variable) that contains multiple slots (the elements) into which the values are stored.
