...
An array is a special kind of **variable**, one that stores multiple values instead of a single value. The values within an array, called elements, all have the same [type](/book/part-1-instructions/1-sequence-and-data/2-trailside/06-type).
...
- You can store a value in an element of an array using an [assignment statement](/book/part-2-organised-code/5-working-with-multiples/2-trailside/00-02-assign-element).
- You can use the value from an element of an array in an [expression](/book/part-2-organised-code/5-working-with-multiples/2-trailside/00-03-expressions-with-arrays).
- The [for loop](/book/part-2-organised-code/5-working-with-multiples/2-trailside/01-for-loop) works perfectly with an array, you can repeat an action *for each element of an array*.
...
Arrays store multiple values, with an index that provides access to the individual elements within the array. Conceptually this can be viewed as a [variable](/book/part-1-instructions/1-sequence-and-data/2-trailside/07-variable) that contains multiple slots (the elements) into which the values are stored.
...
The size of each of the elements of the array can then be used to quickly locate it, given its index. If you have an array of `integer` values, then these are each 32 bits (4 bytes), so the element at index 3 is 3 × 32bits (96 bits) past the start of the array.
