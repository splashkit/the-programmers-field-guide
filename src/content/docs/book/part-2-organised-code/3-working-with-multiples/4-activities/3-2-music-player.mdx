...
- Update a song
  ...
  The user is asked to enter the name to search for, and is then shown a numbered list of all the songs that contain that text to choose from. They enter the number of the song they want. (Check SplashKit's [contains](https://splashkit.io/api/utilities/#contains) function for this)
...
- Play a song
  ...
  The user selects a song as with update. This will then update the times played, and use the [play music](https://splashkit.io/api/audio/#play-music-named) procedure to start the song playing. You can pass `play_music` the song's unique id.
