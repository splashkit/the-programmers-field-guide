...
- a number of songs (dynamic array)
...
- Add a new song
  ...
  The user will be asked to enter the name, path, and loved status of the song. The number of times played will be initialised to 0, and the unique identifier will be system generated.
  
  The song data will be allocated a unique identifier, using the next id from the music player. For example, if the next id is 0 then the song's identifier will be "song-0". Once this is allocated, the music player will move to the next id - incrementing it by one.
...
- Update a song
  ...
  The user is asked to enter the name to search for, and is then shown a numbered list of all the songs that contain that text to choose from. They enter the number of the song they want. (Use the [contains](https://splashkit.io/api/utilities/#contains) function to check if one string contains another.)

  The user is then shown the details of the song, and a list of options to choose from. These include:
...
- Play a song
  ...
  The user selects a song (using the same process as described above). This will then update the times played, and use the [play music](https://splashkit.io/api/audio/#play-music-named) procedure to start the song playing. You can pass `play_music` the song's unique id.
...
Ensure that you continue to make use of functions and procedures to organise your code, and appropriate control flow structures within these.
