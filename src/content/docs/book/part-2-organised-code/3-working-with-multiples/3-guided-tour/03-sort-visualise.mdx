...
Once we start storing arrays of elements, a common operation we want to perform is sorting them! Array sorting has a wide range of applications. In this tour we'll write a program that visualises this process graphically, so we can see how these work on the elements within the array.

Our final program will look something like this (though we will focus on sorting more values):
...
_Customise the colours - maybe matching color toheight :)_

## Sorting arrays

Let's consider the case where we have an array of integers. We might have an array of elements like so:
...
1. ### Set up our array
   ...
   1. First, open up a new `.cpp`, and add an empty `main` function.
   2. Now let's declare an array to sort. For this tour we'll try using a `fixed_array`, that stores `int` and has 10 elements. Make the number of elements a constant outside of `main` called `DATA_SIZE` - that way we can access and/or change it later easily. Make sure to include the `"splashkit-arrays.h"` header as well!
   3. Next, set values in the array using `array[index] = value`. Just put in some random numbers between 0 and 10 for now.
...
2. ### Visualising our array
    ...
    2. Create a `visualize_array` procedure with a single parameter - one for our array (`const fixed_array<int, DATA_SIZE>& data`). Inside it, just clear the screen to white, and refresh the screen.
    3. In `main`, call `visualize_array` ( passing in the array ), then delay for two seconds.
    ...
    - fill each box using `fill_rectangle`.
    ...
    <Accordion>
      ...
      >
        ...
        Bar Width = SCREEN_WIDTH / element count (size)

        For each element:
            Bar Height = element value / MAX_VALUE * SCREEN_HEIGHT
            x = element index * Bar Width
...
3. ### Bubble Sort - A Single Pass
    ...
    Notice how even though the `fixed_array` knows it's own size, we're still passing in a `range`?

    The bubble sort pass itself doesn't always have to loop over the entire array. For instance, if we've already completed three passes, then we know the last three elements in the array are already sorted! So we will use the `range` parameter to only loop over the necessary elements, rather than the using the size of the entire array.

    While we'll be looping using the `range`, we do however still need the full size of the array anyway, so that we can call `visualize_array` and see the entire array changing.

    :::caution
    Just a side note on regarding this:

    Ideally, sorting logic should not depend on rendering at all - we are mixing together two entirely different tasks. Normally this would be bad design! We are only doing this for the purpose of learning, try to avoid this sort of design in general :)
    :::
    ...
    Try calling `bubble_sort_pass` inside `main` (for now pass the array's full length to `range`), and you should now be able to see the array changing as the larger columns move to the right! The largest element should end up all the way on the right side - make sure this is working right before continuing.
...
1. ### Dynamic highlighting
    ...
    Inside `visualize_array`, we now test the `highlight_index` when calling `fill_rectangle`, and change the colour.
...
2. ### Larger array
    To explore sort algorithms in more detail, we need to see many more values.
    Next, lets try increasing the size of our array. Try changing the `DATA_SIZE` to be your `screen_width()` :)

    This will mean that each bar is now a single pixel wide - working with a screen that is 1024 or 1280 pixels wide will help you really see some differences as you explore other algorithms.

    We can no longer populate this manually. Switch the code to use a `for` loop to fill it with random numbers. Have a go at writing the following procedure:

    |   |   |
    |---|---|
    | **Procedure** | *Fill Array Random* |
    | **Parameters** | `data`: The `fixed_array<int, DATA_SIZE>` to fill. |
    | **Description** | Fills the array with random integers between `0` and `screen_height()`. |

    Use the SplashKit function `int rnd(int min, int max);` to help with this. Once you get this working, you should be able to try out visualising the bubble sort with a thousand+ elements!
</Steps>
...
There are also many other sorting algorithms that you could try implementing - personally I like [Merge Sorts](https://en.wikipedia.org/wiki/Merge_sort). Playing around with these will really get you thinking about different ways of achieving the same outcome.

:::tip

Remember that your drawing code will be by far the slowest part of the algorithm. When playing with this, to really see the impact of the different algorithms, you want to make sure you are drawing within each inner iteration. As you explore different algorithms, pay attention to where you are drawing.
