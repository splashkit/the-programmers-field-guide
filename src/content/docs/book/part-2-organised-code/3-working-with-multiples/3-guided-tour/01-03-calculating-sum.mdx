...
- See how we use the [for loop](/book/part-2-organised-code/3-working-with-multiples/move-0-panorama/1-array-iteration) to iterate over the elements in the array. In this case we add the value from the current element to the `total` to calculate the sum.
