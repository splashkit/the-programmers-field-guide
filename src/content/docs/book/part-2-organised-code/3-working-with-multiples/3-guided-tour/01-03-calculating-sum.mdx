...
- See how we use the [for loop](/book/part-2-organised-code/3-working-with-multiples/5-reference/02-00-for-loop) to iterate over the elements in the array. In this case we add the value from the current element to the `total` to calculate the sum.
