...
1. ### Set up our array
   ...
   1. Declare an `array` of `int`s, with 10 elements. Make the number of elements a constant inside `main` - that way we can change it later easily.
   2. Next, have a go at using [list initialization](/book/part-2-organised-code/3-working-with-multiples/5-reference/00-01-array-declaration/#example) to initialise the values in the array. This is the same as what we used for structs (` = {...}`). Just put in some random numbers between 0 and 10 for now.
    ...
    <Accordion>
      ...
      int main()
      {
          ...
          int data[DATA_SIZE] = {2, 7, 9, 2, 7, 3, 10, 3, 2, 1};
...
2. ### Visualising our array
    ...
    2. Create a `visualize_array` procedure with two parameters - one for our array (`int data[]`), and one for the _size_ of the array (`int size`). Inside it, just clear the screen to white, and refresh the screen.
...
3. ### Bubble Sort - A Single Pass
    ...
    | **Parameters** | `data`: The `int` array to sort. |
    ...
    **However**, we still need the full `size` of the array, so that we can call `visualize_array` and see the entire array changing.
    ...
    <Accordion>
      ...
      >
        ```c++
        void bubble_sort_pass(int data[], int size, int range)
        {
...
2. ### Larger array
    ...
    | **Parameters** | `data`: The `int` array to fill. |
    |                | `size`: The size of the array. |
