---
title: Adding Generics
sidebar:
    label: " - Adding Generics"
---
...
It's great that we have a `double_array` class, but what if we want a `book` or `int` array instead?

In previous chapters, copy/pasting the class and adjusting it for each type is the best we could have done. We'd copy the class, rename it, and then change all the `double` to, for instance, `book`.

Of course, we now have _generics_! Let's see how we can apply them to the `double_array` class, to make it support _any_ type at all.
...
<TemplateInjector replace={[
    ...
    {selector: "del", template: 1, contains: "void"},
    ...
    ```c++ frame="terminal" title="Select Inside this Code!" ins="double" ins="double_array" del="MAX_CAPACITY" ins="50" del="public" del="int" del="void" del="class" del="const"
    ...
    <template> <ITO seamless group="void"><slot/></ITO></template>
...
<mistake-reason group="void">
:::note[Not Quite]
    No need to make these methods return anything :)
:::
</mistake-reason>
...
We'll continue working from the code you wrote on the last page, with the same test `main` as well. Feel free to make a copy of the file to continue working from.
...
1. ### Making it generic
    ...
    :::tip
        Declaring a template parameter list looks like this:
        ```c++
