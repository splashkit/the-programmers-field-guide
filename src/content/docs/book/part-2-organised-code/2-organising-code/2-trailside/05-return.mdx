...
For other examples, have a look back at the [example using parameters](/book/part-2-organised-code/2-organising-code/2-trailside/04-parameter). The `square` function uses the return statement to return the squared value, while `point_in_circle` returns the value from the comparison to indicate if the point is within the circle.
