...
The return statement is a kind of [jump statement](/book/part-1-instructions/3-control-flow/1-concepts/05-0-jumping) that ends the current [function](/book/part-2-organised-code/2-organising-code/1-concepts/04-function-decl) or [procedure](/book/part-2-organised-code/2-organising-code/1-concepts/02-procedures). This is useful for skipping the rest of function or procedure's block, and returning early to the calling code. In a function, you also use the return statement to provide the value that is sent back to the caller.
...
- Return statements should be combined with [branching](/book/part-1-instructions/3-control-flow/1-concepts/03-0-branching) statements to make sure the function or procedure is ended only when necessary.
- In a function, a return statement **must** have an expression.
- In a procedure, a return statement **must not** have an expression.
