...
The return statement is a kind of [jump statement](/book/part-1-instructions/3-control-flow/2-trailside/05-0-jumping) that ends the current [function](/book/part-2-organised-code/2-organising-code/1-concepts/04-function-decl) or [procedure](/book/part-2-organised-code/2-organising-code/1-concepts/02-procedures). This is useful for skipping the rest of function or procedure's block, and returning early to the calling code. In a function, you also use the return statement to provide the value that is sent back to the caller.
...
When you use a return statement to exit a function or procedure early, you need something like an [if statement](/book/part-1-instructions/3-control-flow/2-trailside/03-1-if) that checks if you should exit, and then put the return statement within the body of the if. This can be used when you know that the procedure or functions only works in certain circumstances. For example, in the [change calculator](/book/part-2-organised-code/1-starting-cpp/2-put-together/02-change-calculator) we could exit early if the user has not paid enough for the item, or if they have provided the exact right amount.
...
For other examples, have a look back at the [example using parameters](/book/part-2-organised-code/2-organising-code/1-concepts/04-parameter#functions-with-parameters). The `square` function uses the return statement to return the squared value, while `point_in_circle` returns the value from the comparison to indicate if the point is within the circle.
...
- Return statements should be combined with [branching](/book/part-1-instructions/3-control-flow/2-trailside/03-0-branching) statements to make sure the function or procedure is ended only when necessary.
- In a function, a return statement **must** have an expression.
- In a procedure, a return statement **must not** have an expression.
