...
In this tour we will explore the use of `malloc` to create an array of double's in memory. We are focusing on the basics of using `malloc` and `free` here, and how these can be used to allocate space in memory. The program we create will be very simple, just creating an array of a variable size, populate it with double values, and then print these values out. In the [advanced tour](/book/part-2-organised-code/6-deep-dive-memory/2a-advanced-tour/01-overview.md) we will look at making this into a more usable utility, so focus on the mechanics here.
...
int main()
{
    ...
    size = read_int("Enter the number of items to store: ");
