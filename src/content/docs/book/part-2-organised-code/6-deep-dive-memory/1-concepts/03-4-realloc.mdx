...
export const sliderReallocContiguous = [
  ...
  },
  {
    ...
    tips: [
      ...
      "Again, note that the expanded memory area is not initialized to 0 by <strong>realloc</strong>"
    ]
  },
  {
...
### Realloc (contiguous allocation)

Navigate the slide-show below to understand how realloc works when the memory reallocation happens to be contiguous
<br/>
<MySwiper client:only height="" images={sliderReallocContiguous}></MySwiper>

### Realloc (non-contiguous allocation)
Navigate the slide-show below to understand how realloc works when the memory reallocation happens to be contiguous
<br/>
