...
export const sliderReallocContiguous = [
  ...
  },
  {
    ...
    altText: "At line 9, we call realloc again to grow the memory area pointed to by p from 1 integer to 5 integers. When it can, realloc will grow the memory area using the same original location. You can see here, that this has happened, and that the same memory address has been returned and assigned to new_p that was originally allocated to p. Again, note that any uninitialized elements in the the expanded memory area are not initialized to 0 by realloc",
    tipStart: 5,
    tips: [
      ...
      "You can see here, that this has happened, and that the same memory address has been returned and assigned to <strong>new_p</strong> that was originally allocated to <strong>p</strong>",
  ...
  },
  {
    ...
    altText: "Now let's try assigning a new value to the 4th element of our memory region (which contains an array of ints)",
    tipStart: 8,
    tips: [
      "Now let's try assigning a new value to the 4th element of our memory region (which contains an array of ints)"
    ]
  },
  {
    ...
    tips: [
      ...
      "<strong>NOTE:</strong><ul><li>That <strong>p[0]</strong> contains our original value of <strong>1</strong></li><li><strong>p[3]</strong> contains our newly initialized value of <strong>9</strong></li><li>And that any elements that were not initialized will contain an uncertain value (although here they happen to be 0) </li></ul>"
    ]
  },
  {
...
export const sliderReallocMoved = [
  ...
  },
  {
    ...
    altText: "At line 9, we call realloc again to grow the memory area pointed to by p from 1 integer to 5 integers. When it can, realloc will grow the memory area using the same original location, but if it can't it will allocate a new space in memory. You can see that in this case, a new memory space has been allocated and that any values from the original memory area have been copied over to this new space. new_p is assigned the address of this new memory region and the old memory region is de-allocated. Again, note that any uninitialized elements in the the expanded memory area are not initialized to 0 by realloc",
    tipStart: 5,
    tips: [
      "At line 9, we call <strong>realloc</strong> again to grow the memory area pointed to by <strong>p</strong> from 1 integer to 5 integers",
      ...
      "You can see that in this case, a new memory space has been allocated and that any values from the original memory area have been copied over to this new space",
