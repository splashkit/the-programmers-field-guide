...
export const sliderMalloc = [
  ...
  },
  {
    ...
    altText: "At step 4b, malloc() is called and memory is allocated on the heap for an integer. We've shown the bit-representation of an integer with value 0 to help you see the actual low-level memory allocation that is performed in this step The value of the integer will be initialized to 0.  malloc() will return a void pointer",
    tipStart: 3,
    tips: [
      "At step 4b, <strong>malloc()</strong> is called and memory is allocated on the heap for an integer",
      "The value of the integer will be initialized to 0<ul><li><strong>NOTE:</strong> We've shown the bit-representation of an integer with value 0 here to help you see the actual low-level memory allocation that is performed in this step</li></ul>",
      "<strong>malloc()</strong> will return a <strong>void</strong> pointer..."
    ]
  },
  {
    src: sliderMalloc04.src,
    altText: "Because malloc() returns a void pointer, we need to cast it to the type of pointer our variable will hold, in this case an int pointer (step 4c)",
    tipStart: 4,
    tips: [
  ...
  },
  {
    ...
    altText: "Line 5 prints the address of the memory space pointed to by p",
    tipStart: 6,
    tips: [
      "Line 5 prints the address of the memory space pointed to by <strong>p</strong>"
    ]
  },
  {
    src: sliderMalloc07.src,
    altText: "Line 6 prints the dereferenced value of  p, in this case 0 (we've gone back to decimal representation now for simplicity)",
    tipStart: 7,
    tips: [
      "Line 6 prints the dereferenced value of  <strong>p</strong>, in this case 0 (we've gone back to decimal representation now for simplicity)"
    ]
  },
  {
    src: sliderMalloc08.src,
    altText: "Line 7 assigns the dereferenced value of p, the new value of 10. You can see that the value has changed from 0 to 10 on the heap",
    tipStart: 8,
    tips: [
      "Line 7 assigns the dereferenced value of <strong>p</strong>, the new value of 10",
      "You can see that the value has changed from 0 to 10 on the heap"
    ]
  },
  {
    src: sliderMalloc09.src,
    altText: "Line 8 prints the new dereferenced value of p",
    tipStart: 9,
    tips: [
      "Line 8 prints the new dereferenced value of <strong>p</strong>"
    ]
  },
  {
  ...
  },
  {
    ...
    altText: "To avoid problems with any later use of variables that have acted as pointers, the best practice is toto reset the value of p to a NULL value. We do this by assigning p> the value nullptr, which is a which is a a built in constant that is a shorthand for a null pointer",
    tipStart: 12,
    tips: [
      "To avoid problems with any later use of variables that have acted as pointers, the best practice is toto reset the value of <strong>p</strong> to a <strong>NULL</strong> value",
