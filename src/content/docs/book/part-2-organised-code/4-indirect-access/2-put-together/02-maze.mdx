...
Check out the code in the [printing and scanning](/book/part-2-organised-code/4-indirect-access/1-concepts/03-printing-scanning) section, or copy in your code that uses `read_line` from SplashKit. This function will ensure that the user enters a number so that we can determine which path they want to take.
