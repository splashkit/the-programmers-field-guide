...
For this tour, we'll be expanding upon what we learned in [Explore Structs (GUI)](/book/part-2-organised-code/2-structuring-data/3-guided-tour/0-3-explore-struct/), to create a small game where the player controls a spider on a web and catches flies. We'll focus on using _composition_, that is, structs within structs, to help us model our game's digital reality in a more meaningful way. By the end of this, you should feel more confident when structuring your programs, and understand various ways `struct`s can be used and designed.
...
3. What do we need to _support_ our digital reality? How will we write our program so that we can display our spider and fly, and enable the user to interact with it?
    ...
    <AccordionItem header="Let's check...">
        ...
        - a loop that handles input events, so that we can continuously redraw the screen after user input.
...
3. ### Adding `new_spider` and `new_fly`
    ...
    Brace initialization is absolutely fine for simple static types (like `color`, which has an `r` `g` `b` and `a`, or `point_2d` which has an `x` and `y`), but as structures grow it becomes fragile and harder to read. Besides, we will want to add logic that runs when initializing the fly and spider, so it makes more sense to move this initialization elsewhere.
    ...
    This should be familiar after writing `new_target` in [Explore Structs (GUI)](/book/part-2-organised-code/2-structuring-data/3-guided-tour/0-3-explore-struct/). Have a go at writing these functions, and adjusting your initialization code in `main` accordingly.
