...
2. ### Generalizing our entities
    Now for something more involved - let's find some commonality between `fly_data` and `spider_data`, and structure our data in a richer way. Below are the important pieces of code that use them - have a go at identifying the four main things that they all _have_ and _do_ in common:
    ...
    2. Instead, we'll create a _new_ struct called `entity`. `entity` will contain all the data that we found in common so far:
        ...
        - the radius (a `double`)
        - the colour (a `colour` - also from SplashKit)
        ...
        :::tip
            Our `entity` struct currently contains everything needed to _draw_ the entity, but we'll soon start adding fields to control _behaviour_ as well.
        :::
    ...
    4. Make sure to then update how `game.spider` and `game.fly` are initialized in `new_spider` and `new_fly`. You can initialize the fields on seperate lines, or _nest_ the initialization like so:
...
The digital reality we are constructing is now starting to become richer - rather than having flies and spiders be entirely seperate things, we've grouped them into a `game_data`, and shared common data and functionality between them via `entity`. Now let's continue on to actually moving the spider around.
...
<Steps>
1. Let's handle this by writing a `move_with_keyboard` procedure. It will take an `entity` called `ent` by reference, and a `speed` (a double). We can then call it like `move_with_keyboard(game.spider.ent);` inside the main event loop.
...
2. Hopefully now our spider (well...circle) is moving around! However, it's very easy to send the spider off screen if we hold down a key long enough. Now let's try and constrain the spider to stay on screen.
    ...
    We'll need four checks for this, one for each edge of the screen. Have a go at that, and now your spider should always remain on screen.
</Steps>
...
```c++ "fly"
move_with_keyboard(game.fly.ent, SPIDER_SPEED);
```
