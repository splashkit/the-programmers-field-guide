...
2. ### Generalizing our entities
    ...
    2. Instead, we'll create a _new_ struct called `entity`. `entity` will contain all the data that we found in common so far:
        ...
        :::tip
            For this tutorial, `entity` represents a physical presence. Right now we'll only use these fields for drawing, but later on we'll use the `radius` and `position` for checking collisions as well.
        :::

        Notice that we're not grouping these fields _just_ because they were duplicated. This grouping itself has meaning, and helps us construct our digital reality - we shouldn't just put everything inside `entity` without thinking it through first.

        That said, having this `entity` concept will make it really easy to add more fields that all our entities need - for instance, later on we'll give them an `image` field :)

        :::note[Is `entity` the best approach here?]
        <Accordion>
        <AccordionItem header="Would other structures work?">
        For this project `entity` works well, but there are definitely other valid approaches:
         - For instance we could group `position` and `radius ` into a `collision_circle` struct, and place `color` and a second `radius` inside a `renderable_circle` struct.
         - `spider_data` would then look like this:
            ```c++
            struct spider_data
            {
                collision_circle collision;
                renderable_circle renderable;
            }
            ```
         - We would then create a new struct for images, `renderable_image`, and replace the `renderable_circle` field with it.

        This would also be a perfectly valid way of structuring this, and has it's own pros and cons.
        </AccordionItem>
        </Accordion>
        :::
