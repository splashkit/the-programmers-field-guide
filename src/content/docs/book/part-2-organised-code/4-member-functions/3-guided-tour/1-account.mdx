...
:::info
Remember the main difference in C++ between a struct and a class is that class members are private by default. We will review this in depth in Part 3.
:::
...
Now we can update main to test out this new class. Try creating different account objects on the stack, and get them to print out their messages.
...
Objects are values created from a struct or class type, where we are using member functions. As with these values before, they can exist on the stack and follow the same scoping rules.
...
Notice how you can control access to the private `balance` field. Here we have a `get_balance` method that lets you read the value, and `deposit` and `withdraw` methods to allow you to change the balance. But, you cannot just set the balance to be any value you want. It has to be done through those methods.
