...
Here is the same example we worked through on the [if statement](/book/part-1-instructions/3-control-flow/5-reference/03-1-if#multiple-if-statements-combined) page. To get this working, we need to use the SplashKit library to get access to `read_line` and `write_line`.
