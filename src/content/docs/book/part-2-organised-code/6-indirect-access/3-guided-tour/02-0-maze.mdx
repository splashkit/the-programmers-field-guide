...
To get started, we can build part of the program, while exploring how paths could be created to move to a destination.
...
Each room will have a *title* and a *description*, and in the next page we will add a number of paths to other locations.
...
To do this, we can [forward declare](/book/part-2-organised-code/1-structuring-code/5-reference/07-forward-declarations/) the `room_data` struct. We can also create a [`typedef`](/book/part-2-organised-code/2-structuring-data/5-reference/03-00-type-declaration/#example) and set up an alias for a pointer to a `room_data`. We can name this type `room_ptr`. The code for this is shown below.
...
For now, our main objectives are to create rooms and paths. We'll add proper logic for moving in the next page when the expand the structs further.

For this we will need to create:

- A constructor for `room_data` - to populate a room with a title and description.
- A constructor for `path_data` - to populate a path between rooms.

- `print_path` method - to output details of a path as part of a list the user can choose from
- `print_room` method - to output details of a room to the user.
...
This constructor will initialize the path's fields. While we could just use default brace initialisation, we will want to be able to expand the struct going forward and this will mean changes to the way things are initialised. Having a constructor that is responsible for this means we only have one place we need to update when we want to change how initialisation works.

Below is the header comment for the constructor. This captures the requirements for what this will need to do. In the code, you will need to set the `path_data`'s title and descriptions.
...
Like the path constructor, this constructor will populate a room struct with the required data. Below is the header comment for the constructor.
...
Print path will be a `path_data` method that outputs a path's details to the terminal. This will always be printed in a list, so we can accept a parameter for the index of the path to include. This can then be used by the user to determine which path they want to take.
...
This will also be a simple `room_data` method at this stage. We can print the title, and room description. In the next page we will need to expand this to also display the paths from the room.
