...
<Accordion>
  <AccordionItem header="My code for if you get stuck">
    ...
    The method inside `linked_list<T>` takes another template parameter `R`. The `func` parameter is also adjusted to take a parameter of type `R`, and we've made the `T` reference `const` now.
...
A lambda starts with `[]`, and then just looks like the parameters and body of a normal function definition that we can write _inline_ within our function parameters. In this case, the lambda is being converted into a function pointer, and that pointer is passed into `visit`. Already this is a bit more usable!
...
By default, the inside of the lambda is an entirely separate function to the outside. For instance, in the following code, we couldn't access `multiply_by` inside the lambda:
...
We also don't know what _type_ the lambda is - it does have a type, but there's no way to write it. So how do we pass it in `visit`?
...
Here, `sum` is capture by reference, so when it's modified in the lambda, it becomes modified in outer scope as well.
