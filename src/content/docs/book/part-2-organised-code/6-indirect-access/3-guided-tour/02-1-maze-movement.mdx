...
The main limitation currently is that our `main` is filled with logic for handling how each room connects. Let's move our paths to inside `room_data`, and store them inside our [`bounded_array`](/book/part-2-organised-code/5-operators-generics/3-guided-tour/02-00-generic-array/) - this way, each room can have its own set of paths that lead to other locations in the maze.
