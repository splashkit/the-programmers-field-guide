...
If we fail to find a previous node at all, we should throw an exception - you can use any of the methods [previously explained](/book/part-2-organised-code/5-operators-generics/3-guided-tour/02-02-array-class-methods/#add-the-add-method) for this :). This may happen if we are given a `node` that belongs to a different linked list.
