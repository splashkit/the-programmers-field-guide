...
Copilot has us covered for this. For me, Copilot generated the comment and the code. Before we move on we need to make sure that this works as we want. Step through the code, and double check that things will work for the first and all subsequent nodes added to the list.

You can try prompting with this if Copilot does not auto-generate this for you. You may also need to add in the placement new to initialise the C++ data, if needed.
