...
```
_If we want to handle the `const` variant, we would need to do the same thing we did with `get` in [Generic Array](/book/part-2-organised-code/5-operators-generics/3-guided-tour/02-04-array-overloads/#before-we-do-that)_
