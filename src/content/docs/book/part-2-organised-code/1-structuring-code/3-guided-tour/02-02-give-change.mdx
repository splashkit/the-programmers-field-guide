...
5. Now we can make our code  **repeat** to calculate the change, once for each coin. We can use a [while loop](/book/part-1-instructions/3-control-flow/5-reference/04-1-while-loop) to achieve this, as there are a set number of coins that we need to iterate through. This gives us the following pseudocode:

    ```txt
    set i to 0
    while i is less than the number of coins
        ...
