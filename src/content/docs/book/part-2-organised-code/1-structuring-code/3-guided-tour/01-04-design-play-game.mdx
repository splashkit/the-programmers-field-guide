...
The flowchart below shows the logic for play game. This has a *loop* to ask the user to perform up to 7 guesses. The condition on this loop occurs *after* the loop body as the user must have at least one guess - so we can use a [do while](/book/part-1-instructions/3-control-flow/5-reference/04-2-do-while) loop.
