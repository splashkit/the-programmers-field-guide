...
- `draw_heading_line`: accepts a width parameter, and outputs a string like `"+------------------------+"` (when width is 26).
- `draw_title`: accepts a title and width. Outputs the title text in a box (with the heading line above and below). For example, the main menu title above would have the title as "Main Menu", and a width of 26.
...
Use the [length_of](https://splashkit.io/api/utilities/#length-of) function to get the length of a string. You will need this to calculate the number of spaces to add before the | at the end of the title.
:::