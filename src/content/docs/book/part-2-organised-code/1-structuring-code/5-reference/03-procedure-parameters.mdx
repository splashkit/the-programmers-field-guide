...
These values can be passed to the procedure using **parameters**. A parameter is a [variable](/book/part-1-instructions/1-sequence/5-reference/07-variable) that has its value set by an [argument](/book/part-1-instructions/1-sequence/5-reference/02-method#arguments) in the procedure call.
...
Parameters are an optional addition to any procedure declaration -- in between the declarations parentheses you write [variable declarations](/book/part-2-organised-code/1-starting-cpp/2-trailside/4-1-variable-constant) (without initialisation), separated by commas.
