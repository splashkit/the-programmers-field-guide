...
These values can be passed to the procedure using **parameters**. A parameter is a [variable](/book/part-1-instructions/2-data/5-reference/10-variable) that has its value set by an [argument](/book/part-1-instructions/1-sequence/5-reference/10-procedure#arguments) in the procedure call.
