...
With function and procedure calls, we add a new kind of block that can be added to the stack. In addition to local variables, the stack maintains all the data needed to make function and procedure calls work. We saw this a little in the original details on [method calls](/book/part-1-instructions/2-data/5-reference/31-function-call) where we saw how the program counter was maintained on the stack for a single method call.
