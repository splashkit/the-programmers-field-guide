...
With pass by reference you instead pass a variable, a location to store a value. Pass by reference allows you to pass across the variable's location, which allows the function or procedure to read and **modify** the value of the variable that was passed in.
...
- To improve performance when passing large amounts of data.
...
With pass by reference, you can create functions and procedures that can update the variables passed to it. In most cases these will be procedures, where the action performed has a side effect on the parameter passed in. For example, in our "Player Move" program we could create a `handle_input` procedure that is passed the `player_x` and `player_y` to update. In `handle_input`, the code can read and change the data passed to it, allowing it to update the position of the player in response to user input.

### Improve performance

So far we've generally been passing small amounts of data as our parameters, such as invdividual numbers. However, as our programs become more complicated and we learn more concepts, we'll start passing around larger volumes of data - imagine, for instance, an extremely long string, or a list of a million numbers. Copying this data in by value takes time, as the computer has to duplicate all the information (e.g. every character in the string) to copy it by value.

References are achieved by passing the address of the variable passed to it. This is always the same size, and can be passed efficiently to the parameter.
