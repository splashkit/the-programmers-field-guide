...
In this tour we will explore the use of [malloc](/book/part-2-organised-code/7-deep-dive-memory/5-reference/03-0-allocating-memory) to create an array of double's in memory. `malloc` is a function in C's standard library (you need to `#include <cstdlib>`). Like [new](/book/part-2-organised-code/6-indirect-access/5-reference/02-01-new-delete), `malloc` will allocate space on the heap - with the name representing "memory allocation". This is paired with `free` that is used to free the allocation much like `delete`. Unlike `new` and `delete`, with `malloc` you work in bytes... allocating as much or as little space and using it however you want.

We are focusing on the basics of using `malloc` and `free` here, and how these can be used to allocate space in memory. The program we create will be very simple, just creating an array of a variable size, populate it with double values, and then print these values out. In the [dynamic array tour](/book/part-2-organised-code/7-deep-dive-memory/3-guided-tour/02-0-dynamic-array) we will look at making this into a more usable utility, so focus on the mechanics here.
...
Also add in the code to check that this succeeded, and exit the program if you could not allocate the number of bytes requested. The `malloc` function returns `NULL` if it cannot allocate the space that you asked for.
...
Here we are working with C memory allocation functions. These do not do anything to the memory that is allocated, so you have to be very careful if you are using this space with C++ type with member functions.
