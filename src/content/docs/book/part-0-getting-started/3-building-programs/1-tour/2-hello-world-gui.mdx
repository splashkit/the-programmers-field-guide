...
If the `dotnet run` command is not working, make sure you have SplashKit installed correctly. If you were not able to install SplashKit globally, go to the [No SplashKit Global Install](/book/appendix/0-installation/3-0-troubleshooting-install/#no-splashkit-global-install) section on the Installation Troubleshooting page to see the alternate commands to use.
:::
