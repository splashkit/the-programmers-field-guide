...
Continuing on from [the previous page](/book/part-0-getting-started/3-building-programs/2-put-together/1-hello-world-cli), we will now be using *SplashKit* to create a GUI (**G**raphical **U**ser **I**nterface) Hello World program.
...
If the `dotnet run` command is not working, and you were not able to install SplashKit globally, go to the [No SplashKit Global Install](/book/part-0-getting-started/2-computer-use/0-installation/3-0-troubleshooting-install/#no-splashkit-global-install) section on the Installation Troubleshooting page to see the alternate commands to use.
:::
...
:::caution[MSys download issues]
Currently the SplashKit download functions will not function if you are using MSys on Windows. If you want to run these programs, you need to download the images and sound effects manually, save them in the folder with your program, and change the code to use the "Load" version of the methods. For example, `LoadSoundEffect("Hello World", "hello-world-snippet-saddle-club.ogg");`.
:::
