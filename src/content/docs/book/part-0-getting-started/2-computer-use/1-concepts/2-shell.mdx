...
The **Terminal** program provides a text environment, allowing text input and output much like the old physical terminals once did. Something within  the computer needs to receive the text you type in the Terminal and respond to it. We call this program a **shell**.
...
A **shell** program runs within a terminal program to interpret your commands and instruct the kernel to perform the actions you request. The shell is an interactive program that will send prompts to you, displayed  through the terminal, at which you enter your commands by typing text. There are several different shell programs you can choose from, each of which has its advantages and disadvantages. The shell we are going to use for this journey will be **Z shell**. This is a shell built into most modern Unix operating systems.
...
At this point, you should see a “prompt” within the terminal. This is the **shell** asking you to give it an instruction. You type commands, and press Enter or Return to get the shell to do what you have asked.
