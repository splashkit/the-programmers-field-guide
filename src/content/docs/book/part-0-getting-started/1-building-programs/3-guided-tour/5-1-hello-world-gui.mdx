...
1. Create a C++ file
    ...
    <summary>Try structuring your folders too!</summary>
        Now might be a good time to consider how you structure your folders. For instance, you could make a folder for each chapter or task:
        ...
        Just remember to use "Open Folder" in VS Code when you switch between projects! So with the above when you are working on Building and Programs, open the **building-programs** folder. Otherwise `clang++` won't be be able to find your C++ source files.
    </details>
...
2. Copy the following code and paste it into your C++ file:
    ...
    This program is a bit longer than the last one! Don't worry, we'll take a look at what it does in a moment...

3. Now compile and run it!

    I won't put the exact commands here this time, but here's the general _template_ of how it should look. Try filling in the placeholder values between the `<...>` (angle brackets) yourself:
    ```sh
    clang++ <source-file.cpp> -o <program-name> -l SplashKit -Wall
    ./program-name
    ```

    For me, my source file is named `gui-hello-world.cpp`, and I want the program to be called 'hello-gui', so those are the _parameters_ I'll pass to `clang++`, and the name I'll use to run the program.

4. Here is what the code above will create:
...
This has been good practice in building programs, but so far we've mainly just copy-pasted code. Let's try making some more small modifications!

First, let's just try _reading the code_.

:::tip
Learning programming is a lot like learning a new language, except most of the "words" are ones you probably already know!
...
Have a read through those lines, and have a guess at what they're doing. How is each line affecting what the program shows on the screen? Try writing it down :)

<details>
<summary>You've got your guesses? Let's see how they compare:</summary>
    Just from reading the text, we can probably guess it's doing the following things:
    - It **open**s a **window** and somehow the text "My First GUI Program", and the numbers 640 and 480 are used.
    - It **clear**s the **screen** with the colour white.
    - It **draw**s some **text**, coloured black.
    - And it starts drawing a H by **fill**ing in red **rectangle**s.

    We can see how each line of code _does something_, and uses values to change its behavior (for instance what colour to use, what text to write, etc). Here's a (messy) diagram showing how each line maps to the screen:
    ...
    Yes! At the command line, we run a program by writing its _name_, and then a series of _parameters_ (or _arguments_). For instance when we write:

        ```sh
        clang++ gui-hello-world.cpp -l SplashKit -o hello-gui -Wall
        ```

        `clang++` is the program we're running, and the rest of the line contains the parameters. `clang++` takes those inputs, and goes and _does something_ for us (compiles our program).
        ...
        draw_text("Hello World! - using SplashKit shapes:", COLOR_BLACK, 50, 50);
        ```
        `draw_text` is the _procedure_ we're runnning, and the things inside the brackets `()` are the parameters. `draw_text` then goes and _does something_ (in this case, it draws text).
...
You may have guessed this already, but the numbers, `50, 50` here represent the _position_ the text will be drawn at on the screen.
 - The first number is the `x` coordinate. It tells the computer how far to the right from the left edge of the window we want our text.
 - The second number is the `y` coordinate. It tells the computer how far down from the top edge of the window we want our text.

These numbers represent a number of _pixels_. Similarly, `fill_rectangle` also uses pixel coordinates:
```c++
fill_rectangle(COLOR_RED, 50, 100, 20, 100);
```
The `50, 100` are again the x/y coordinates. The last two numbers are the _width_ and _height_, again in pixels.

Take a look at the diagram below for a visualization. Hopefully you can see how each number specifies a position or size on the screen:
...
If you're interested in learning more about drawing graphics, have a look at the guide [here](https://splashkit.io/guides/graphics/drawing-using-procedures/). It's a little advanced right now, but it might be something you can come back to as you learn more :)
</details>
