...
5.  In the Terminal type the following command:
    ...
    This runs the `clang++` compiler, and tells it to read the `hello-world.cpp` files, link with the SplashKit library (`-l SplashKit`), output *my_program* (`-o my_program`), and warn for all issues it identifies (`-Wall`). If interested, check [here](/book/part-0-getting-started/1-building-programs/5-reference/7-0-building-cpp/) to learn more about the command.
