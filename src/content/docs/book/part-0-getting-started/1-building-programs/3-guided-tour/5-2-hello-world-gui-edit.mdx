...
Continuing from the [previous section](/book/part-0-getting-started/1-building-programs/3-guided-tour/5-1-hello-world-gui/), we've gotten started with a GUI Hello World, and are ready to try customizing it.

## Let's change it!

There are some easy ways we can change the code:

* We can _delete_ lines of code to delete the letters (or parts of the letters!)
...
<Steps>
1. First let's delete some code to turn the "HELLO" into "HI"
    ```diff lang="cpp"
    ...
    ```
    _To follow along, delete the red lines, and add the green ones!_

    What we've done here is delete all the code that creates the letters "L L O", and _most_ of what makes the letter `E`. The `E` was drawn with four rectangles, so we'll leave the line of code that draws the left-most vertical rectangle to become our "I", but delete the rest.

    <details>
    <summary>What about `// Draw the E`? What does it do?</summary>
        `// Draw the E` is just a _comment_. It helps _us_ understand the code, but the compiler pretty much just ignores it. You can leave it as `// Draw the E` and it won't have any effect.

        But, it's better to change it to `// Draw the I` of course! We don't want our comments to become out-of-date and contradict the code, since it'll confuse other people (and ourselves in a few weeks!).
    </details>
...
2. Save, re-compile the code, and run it!
    ...
    <summary>Errors show up when I try to compile it!</summary>
        Oh no! We'll definitely cover how to figure out errors later on, but for now feel free to refer to our completed updated code instead:
        ...
        ```
        Try comparing it to your code and see where things might have gone wrong!
        </div>
    </details>

3. Now let's have a go at changing the parameters to change the colours of the background and the letters.

    For instance, anywhere you see `color_red()`, you could replace this with `color_blue()` (or any other colour!).

    :::tip
    Have a look [here](https://splashkit.io/guides/color/splashkit-colors/#color-palette) for all the colours you can use!
    :::

    I'm going to try and make mine a bit Programmer's Field Guide themed. So I'll be changing the colours passed into `clear_screen`, `fill_rectangle`, `fill_triangle`, `fill_ellipse`, and `fill_circle`. See if you can come up with something you think looks nice.

    For this part you'll want to try changing things, testing, then changing things again, testing again, etc. This _iterative development_ is very important, so feel free to take your time playing with it and trying out lots of different changes.

    ![A flow chart showing the iterative development process](./images/iterative-development.png)
    <details>
    <summary>Errors show up when I try to compile it!</summary>
        Here are some tips:
         - If the error isn't too long, have a go at reading it! It might be as simple as a typo that the compiler picked up on.
         - Make small changes. That way you can easily undo (Ctrl/Cmd-Z) whenever an error occurs.
        ...
        - Make sure SplashKit (the _library_ we're using that does the drawing for us) knows the colour! (check the page mentioned above to see what colours you can choose from)
    ...
    Feel free to make yours more exciting!

4. For the last part, we're going to make the letters type themselves out one at a time!
    ...
    We can see there are two clear sections -- one for drawing the `H`, and the other for drawing the `I`.
    ...
    If we copy paste the `refresh_screen()` and `delay(5000)` lines in-between each letter, we can make the program show the current progress of the drawing, then pause before drawing any more. Give this a go! Don't forget to make small changes, saving, compiling, and running your program each time to see their effect.

    <details>
    <summary>I'm stuck!</summary>
        No worries! Try adding the green lines of code exactly as you see here and see if things make more sense:
        ```diff lang="c++"
        #include "splashkit.h"
...
Hopefully from this you feel more confident with looking at and modifying some code. In the next section you'll start learning the concepts more formally, but some of the intuition you built during this section will come in handy.
