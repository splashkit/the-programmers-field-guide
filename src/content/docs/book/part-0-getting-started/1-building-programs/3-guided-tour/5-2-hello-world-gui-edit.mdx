...
1. First let's delete some code - we'll turn the `Hello` into `Hi`
    ...
    <details>
    <summary>What about the `// E`? What does it do?</summary>
        The `// E` is just a _comment_. It helps _us_ understand the code, but the compiler pretty much just ignores it. You can leave it as `// E` and it won't have any effect :)

        But it's still better to change it to `// I` of course! We don't want our comments to become out-of-date and contradict the code, since it'll confuse other people (and ourselves in a few weeks!)
    </details>
