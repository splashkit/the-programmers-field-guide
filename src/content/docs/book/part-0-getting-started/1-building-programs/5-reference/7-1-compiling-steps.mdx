...
:::caution
There are other things you can do with the preprocessor, and we will look at some later, but generally we would suggest avoiding doing too many tricky things with it. Try to keep your code clean and simple so that others can understand it.
:::

For `#include` to work, the preprocessor needs to know where the included files can be found. There are some default paths that are searched, which includes the current directory. You can also add a `-I` option when compiling to add additional search paths.

### Linking

With C/C++ compilers, the assembler creates what are known as **object files**. These contain machine code for parts of the program that need to be connected (i.e., **linked**) together. For example, the code for `write_line` is in the SplashKit library. The object files where `write_line` is used need to know where the SplashKit machine code is so that it can execute it at run time. The linking process creates these connections.

For the linker to work, it will need to know which libraries you used and where to find them. The `-l` option lets you tell the linker which libraries you want linked in, and the `-L` option can be used to indicate where those libraries can be found.
