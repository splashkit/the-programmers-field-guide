...
These responsibilities are coded as **methods** within the class, in the same way we saw in the [member functions](/book/part-2-organised-code/4-member-functions/5-reference/1-2-methods) chapter.

A class can have any number of methods.
Methods inside a class are very similar to the [functions and procedure](/book/part-2-organised-code/1-structuring-code/5-reference/00-0-functions-and-procedures) we coded in procedural programs: they still have an identifier, can have parameters, and can return a value.
Within a class we can also [overload](/book/part-1-instructions/1-sequence/5-reference/11-procedure-call#overloading) methods, just like we could with functions and procedures in C++.
