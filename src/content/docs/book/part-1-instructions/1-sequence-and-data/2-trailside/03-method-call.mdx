...
As the `WriteLine` method comes from the C# library `System.Console`, this example program starts with a [using directive](/book/part-1-instructions/1-sequence-and-data/2-trailside/00-program/#in-c) to tell the compiler to look in the `System.Console` library for any methods that are called.
