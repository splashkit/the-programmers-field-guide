...
export const sliderImages = [
  {
    ...
    tips: [
      ...
      "In this example, we are showing a compiled executable running. Its instructions are in the file on the disk."
    ]
  },
  {
    src: Slide2.src,
    ...
    tipStart: 3,
    tips: [
      "The instructions can be loaded from disk into memory. This gives the CPU easier access to the instructions as memory is much faster to access than the disk.",
      "Every value (based on the CPU architecture) in memory is uniquely addressable. For example, in a 64-bit machine, each address refers to a 64-bit value. So address 0 is the first 64 bits, address 1 is the second 64 bits, and so on. Each instruction in a 64-bit machine will be 64 bits."
    ]
  },
  {
    src: Slide3.src,
  ...
  },
  {
    ...
    tipStart: 11,
    tips: [
      "The process to run the instruction remains the same - load it into the CPU.",
      "Run the instruction."
    ]
  },
  {
    ...
    tips: [
      ...
      "This same process runs on and on, with millions of these instructions running each second."
    ]
  }
];
...
- Store a value in a variable (called [assigning a variable](../08-assignment-statement) a value)
...
Review the following images to see how sequence executed within the computer.
