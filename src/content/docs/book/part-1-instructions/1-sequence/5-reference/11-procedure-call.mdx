...
As you have seen, [procedures](/book/part-1-instructions/1-sequence/5-reference/10-procedure) are constructs that contain instructions to do a task -- sort of like mini-programs. A large part of your code will be calling procedures to get them to do things you need done, in the order you need them done.
...
When you call a procedure, you need to pass a value for each argument. This value can be a [literal](/book/part-1-instructions/1-sequence/5-reference/14-literal) or calculated value known as an [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression).
...
// fill_circle is its name (known as an identifier)
