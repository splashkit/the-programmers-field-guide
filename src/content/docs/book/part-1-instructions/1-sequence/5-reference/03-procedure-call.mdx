...
A method call is an instruction that gets the computer to run the code in a [method](/book/part-1-instructions/1-sequence/2-trailside/02-method). You use the name of the method to identify which method you want to run. Some methods require data, which you must pass as **arguments** to the method as part of the method call.
...
As you have seen, [methods](/book/part-1-instructions/1-sequence/2-trailside/02-method) are building blocks that contain instructions that do a task -- sort of like mini-programs. A large part of your code will be calling methods to get them to do things you need done, in the order you need them done.
...
When you call a Method, you need to pass a value for each argument. This value can be a [literal](/book/part-1-instructions/1-sequence/2-trailside/05-literal) or calculated value known as an [expression](/book/part-1-instructions/1-sequence/2-trailside/04-expression).
...
A method call allows you to run the code in a [method](/book/part-1-instructions/1-sequence/2-trailside/02-method), getting its instructions to run before control returns to the point where the method was called.
...
As the `WriteLine` method comes from the `SplashKitSDK.SplashKit`, this example program starts with a [using directive](/book/part-1-instructions/1-sequence/2-trailside/00-program/#in-c) to tell the compiler to look in the `SplashKitSDK.SplashKit` library for any methods that are called.
Remember that programs run in [sequence](/book/part-1-instructions/1-sequence/2-trailside/01-sequence), so we need to provide this directive *before* writing any code that calls a method from this library.
...
The following code uses three different methods from the [SplashKit](https://splashkit.io) library. You can see the details for these on the [methods page](/book/part-1-instructions/1-sequence/2-trailside/02-method#examples), but the names should give you a good idea of what this does.
...
Some methods return data, which you can use within any calculated value ([expression](/book/part-1-instructions/1-sequence/2-trailside/04-expression)) within your code. The following code demonstrates the use of this with the ReadLine method. The result of this is being stored in a variable and then used to output a message.
...
Line 4 demonstrates the use of the result from calling `RandomColor` within the call to `ClearScreen`. In this case, the value returned from the `RandomColor` method becomes the value for the argument passed to `ClearScreen`. As you would expect, this will clear the screen to a Test color.
