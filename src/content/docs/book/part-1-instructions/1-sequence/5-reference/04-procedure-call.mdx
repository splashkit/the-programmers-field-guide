...
A procedure call is an instruction that gets the computer to run the code in a [procedure](/book/part-1-instructions/1-sequence/5-reference/03-procedure). You use the name of the procedure to identify which procedure you want to run. Some procedures require data, which you must pass as [arguments](#arguments) to the procedure as part of the procedure call.
...
As you have seen, [procedures](/book/part-1-instructions/1-sequence/5-reference/03-procedure) are constructs that contain instructions to do a task -- sort of like mini-programs. A large part of your code will be calling procedures to get them to do things you need done, in the order you need them done.
...
A procedure call consists of a procedure's name followed by an opening parenthesis, then zero or more arguments (delimited by commas), a closing parenthesis, and a semicolon.
...
When you call a procedure, you need to pass a value for each argument. This value can be a [literal](/book/part-1-instructions/1-sequence/5-reference/05-literal) or calculated value known as an [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression).

You can determine the arguments that are needed from a procedure's signature.

## Signature

The signature of a procedure lists its name and details of the arguments you need to pass. The following is an example procedure signature, and its components.
...
```c++
// Example overloaded procedures - all called write_line
...
The following code uses three different procedures from the [SplashKit](https://splashkit.io) library. You can see the details for these on the [procedures page](/book/part-1-instructions/1-sequence/5-reference/03-procedure#examples), but the names should give you a good idea of what this does.
...
Some procedures return data, which you can use within any calculated value ([expression](/book/part-1-instructions/1-sequence/5-reference/05-expression)) within your code. The following code demonstrates the use of this with the ReadLine procedure. The result of this is being stored in a variable and then used to output a message.
