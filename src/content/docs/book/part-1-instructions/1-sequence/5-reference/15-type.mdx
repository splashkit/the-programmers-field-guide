...
Remember that computers are electronic devices and that everything else on top of that is an [abstraction](/book/part-0-getting-started/1-building-programs/5-reference/3-digital-realities-and-abstraction/) that we build. So, how do the tools we have allow us to use different kinds of data within our programs?
...
There are three basic data types available in pretty much every programming language:
...
As you work with code you will need to use types to classify the different pieces of data within your program. This will allow you to determine where you can use the data, and what actions you can perform on it.
...
Whichever type you are working with, you need to make sure that it is suitable to capture the information you need.
...
`char`, on the other hand, describes a _single_ alphanumeric character.
...
For simple conversions, such as between different number formats, languages usually provide a feature called **type casting**. With this you can ask the language to reinterpret data in a new format. This is achieved by placing the type you want within brackets `()` before the value to be converted. For example, `(int)73.5` will give you the integer 73.

For other conversions you can use a [type conversion function](/book/part-1-instructions/2-data/5-reference/30-function/#type-conversion-functions), which we will learn about in the next chapter.

## Example

Let's revisit our table of example expressions [from earlier](/book/part-1-instructions/1-sequence/5-reference/13-expression/#examples), this time adding another column to show what data type each expression is.
...
Identify the types of the following data. Where an expression is given, evaluate the expression and determine the type of the overall value.
...
- Real numbers are represented as **floating point** values. These values have limited precision, supporting only a certain number of significant digits. The default is `double`.
...
<div class="not-content">
  <YouTube id="24tac93iRLo" params="fs=1&modestbranding=1&rel=0"/>
</div>
</div>
