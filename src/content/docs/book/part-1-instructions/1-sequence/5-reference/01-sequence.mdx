...
export const sliderImages = [
  ...
  },
  {
    ...
    tipStart: 9,
    tips: [
      "The last part of running the instruction will be to add 1 to the program counter.",
  ...
  },
  {
    ...
    tipStart: 13,
    tips: [
      "Once this is done, the computer again adds 1 to the program counter.",
...
Instructions flow through the CPU in order, one at a time, similar to a [data stream](/book/appendix/2-computer-use/2-trailside/09-streams).
...
This design process is known as **computational thinking**. You need to think about the available instructions and constructs, and to use them creatively to come up with a sequence of instructions.

This chapter focuses on learning how to **create** the following **constructs**:

- [Programs](/book/part-1-instructions/1-sequence/5-reference/00-program) the user can run.

You will be able to **use** the following constructs that others have created:

- [Procedures](/book/part-1-instructions/1-sequence/5-reference/10-procedure) that can be called to perform an action.
- [Libraries](/book/part-1-instructions/1-sequence/5-reference/30-library) that provide you with constructs others have created.

You will be able to **instruct** the computer to:

- [Call procedures](/book/part-1-instructions/1-sequence/5-reference/11-procedure-call) to run the instructions they contain.
...
One line of source code will generally have several actual instructions.
...
You don't usually need to draw the steps out as we have here, but it helps to be able to visualise them in your mind.
...
- Calculated values ([expressions](/book/part-1-instructions/1-sequence/5-reference/13-expression)) are evaluated before they are used. This process obeys standard mathematical rules (e.g. [BOMDAS](https://en.wiktionary.org/wiki/BOMDAS#:~:text=English-,Phrase,division%2C%20then%20addition%20and%20subtraction)).
