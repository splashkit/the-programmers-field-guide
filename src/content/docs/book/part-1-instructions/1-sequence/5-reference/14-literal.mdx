...
This can be either a **number value** or **text value**, and can be a whole, or part of, an [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression).
...
Your instructions need data, and one way you can provide it is by literally typing data into the code. A literal on its own is a valid expression, but literals can also be used in expressions to help you calculate new values.
...
1. **Numbers**: a number can have a sign, and can be *whole* or *real*. Whole numbers have no decimal places, and real numbers can have decimal places. For example, `127`, `-8711`, and `+10` are all whole numbers. `3.1415` and `-2.78349` are both examples of real numbers.
...
For each of the following, identify if the value listed or used in a procedure call is a literal or not and, if it is, whether the data is text or a number:
...
<details>
  ...
  <ul>
    ...
    <li><strong>4: </strong>This procedure call uses the literal text "Hello World".</li>
    ...
    <li><strong>8: </strong>Both 10 and 15 are literal numbers, but as a whole this is not a literal. It is malformed because of the space in the middle.</li>
...
* A literal can be part or all of an [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression).
