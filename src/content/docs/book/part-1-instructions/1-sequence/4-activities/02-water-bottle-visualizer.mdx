...
In the section on [variables](/book/part-1-instructions/1-sequence/2-trailside/07-variable#example-using-multiple-variables) we saw how to calculate the volume of water in a bottle. For this program, let's see how we could use SplashKit to draw this to the screen.
...
Opening the window is a call to the `open_window` method in SplashKit, so it is straight forward. We just need to think about the title we want, and how large it will be.
