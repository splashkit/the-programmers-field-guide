...
As we learned in [literals](/book/part-1-instructions/1-sequence/2-trailside/05-literal), a literal string value is enclosed in double quotes (`"`), and a character value is enclosed in single quotes (`'`).
