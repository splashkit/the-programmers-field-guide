...
As we learned in [literals](/book/part-1-instructions/1-sequence/5-reference/05-literal), a literal string value is enclosed in double quotes (`"`), and a character value is enclosed in single quotes (`'`).
...
To help achieve this programming languages provide utility functions that will allow you to convert between different types. In C++, `#include <string>` provides many functions you can use (see details on [cppreference.com](https://en.cppreference.com/w/cpp/header/string.html#Numeric_conversions)). The main ones that we will use are:

- `stoi` - this can convert a `string` to an `int` (**S**tring **to** **I**nt).
- `stod` - this can convert a `string` to a `double` (**S**tring **to** **D**ouble).

For example, `stoi("73")` will give you the integer 73 as its result. Similarly, `stod("73")` would give you the double `73.0`. Note that these will cause the program to crash if the data is not in the right format, we will look later at how to deal with this. For the moment, make sure you enter the data in the expected format.

SplashKit also provides convenient helper functions for this:

- [`to_integer`](https://splashkit.io/api/utilities/#to-integer) - can be used to convert a string to an integer.
- [`to_double`](https://splashkit.io/api/utilities/#to-double) - can be used to convert a string to a double.
