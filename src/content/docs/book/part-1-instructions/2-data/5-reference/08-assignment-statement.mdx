...
Most assignment statements are written using `=`, with an identifier on the left-hand side and an [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression) on the right-hand side.
...
- The right-hand side of the assignment operator is an [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression) representing the value to be stored.
