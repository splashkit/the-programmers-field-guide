...
This could be a [literal](/book/part-1-instructions/1-sequence/5-reference/14-literal), a [function call](/book/part-1-instructions/2-data/5-reference/03-function), or any other [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression).
...
The ordering of these actions allow you to update the value of a variable. This can be very useful. For example, you might want to update the value of a variable storing the number of steps you have taken today. The following code creates a variable called `count`, initialises it to 0, and then updates its value.
...
In C++ the assignment operator is `=`, with an identifier on the left-hand side and an [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression) on the right-hand side.

The assignment operator can optionally be modified with `+`, `-`, `*`, or `/`, which are shorthand for adding to, subtracting from, multiplying, or dividing the variable identified on the left-hand side of the statement.
