...
The assignment statement performs two actions. First, it calculates the value of the [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression/) on the right-hand side of the assignment operator (the `=`). Once it has the value, it stores the value (assigns it) to the variable on the left-hand side of the assignment operator.

![Illustration of assignment code and actions](./images/assignment-statement-pano.png)
...
The [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression) on the right right-hand side of `=` determines the value to be stored.
This could be a [literal](/book/part-1-instructions/1-sequence/5-reference/14-literal), a [function call](/book/part-1-instructions/2-data/5-reference/31-function-call/), or any other [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression).
On the left-hand side, you write the identifier of the variable you want to store this value in.

### Assigning a string value

The following code creates a variable called `name` and assigns it the value "Fred":
...
The order of the actions in the assignment statement allows you to update the value of a variable, even if the expression refers to the variable you are updating. The following code creates a variable called `count`, initialises it to 0, and then updates its value by adding 1 to its current value.

```c++
// Create and initialise count
int count = 0;
...
:::tip[Syntax]
![The C++ basic syntax for an assignment statement](./images/simple-assignment.png "The syntax for an assignment statement")

<CommmonLink type="syntaxDiagramGuide"/>
:::

At the basic level, the assignment statement uses `=` operator. To the left you put the variable name, to the right you put the value you want to store in it.

## Example

In this example we create a `name` and a `message` variable, and assign these values.
...
int main()
{
    ...
    // |   "Niroshinie" is the right-hand side, a string literal.
    // |      |
    // v      v
    name = "Niroshinie";

    // Assign a value to message. The right-hand side concatenates
    // a string literal, with the value from the name variable.
    message = "Hello " + name;
...
The good thing is that you do not usually need to think about these smaller steps, as long as you remember that the assignment statement works by evaluating the expression on the right-hand side of the `=` and then stores the result in the variable on the left-hand side.
...
- The right-hand side of the assignment operator is an [expression](/book/part-1-instructions/1-sequence/5-reference/13-expression) calculating the value to be stored.
