...
export const sliderImagesAssignInt = [
  ...
  ,
  {
    ...
    tips: [
      ...
      "This is just the value 21, so no calculations are required."
    ]
  }
  ,
  {
  ...
  {
    src: Slide12int.src,
    altText: "Let's run through it quickly: 4a. The expression in this case is just the value 4, so it can simply be saved into the variable.",
    tipStart: 12,
    listType: "ul",
    tips: [
      "Let's run through it quickly:<ul><li>4a: evaluates the expression - in this case it the value 4.</li><li>4b: store the result of calculating the expression into the 'divisor' variable.</li><li>4c: The program counter is incremented to line 5.</li></ul>"
    ]
  }
  ,
  {
  ...
  ,
  {
    ...
    listType: "ul",
    tips: [
      "To calculate the expression, the values are loaded (5a and 5b).</ul>"
    ]
  }
  ,
  {
    src: Slide15int.src,
    altText: "The CPU performs the integer division, so 21 / 4 = 5. The result of the expression is stored in a register (Register 3 here)",
    tipStart: 15,
    listType: "ul",
    tips: [
      "The integer division is performed (21 / 4 = 5)",
    ]
  }
  ,
  {
    ...
    listType: "ul",
    tips: [
      "The result is <span class='boldunderline'>stored</span> in the 'quotient' variable.",
  ...
  {
    src: Slide17int.src,
    altText: "Argh... lots of steps here ! We must evaluate the arguments to pass to write_line. 6a: Load dividend 6b: Convert to string 6c: Load '/' 6d: Append 6e: Load divisor 6f: Convert to string 6g: Append 6h: Load '=' 6i: Append 6j: Load quotient 6k: Convert to string 6l: Append. Result is '21/4=5'. Result passed to write_line(k) which outputs it to the terminal",
    tipStart: 17,
    listType: "ul",
    tips: [
      "There are lots of steps here evaluating the argument passed to write_line.",
      "Loading 'dividend', then converting it to a string, then loading and appending the string literal ' / ', and so on...",
      "Overall argument value is the string '21 / 4 = 5'.",
    ]
  }
  ,
  {
...
export const sliderImagesAssignDouble = [
  ...
  },
  {
    ...
    tipStart: 3,
    tips: [
      "To <span class='boldunderline'>evalulate</span> the right-hand side expression, the CPU needs to first <span class='boldunderline'>load</span> the dividend and divisor (5a, 5b).",
      "Next it performs the <span class='boldunderline'>division</span> (5c), with the result 5.25.</li></ul>"
    ]
  },
  {
  ...
  },
  {
    ...
    tips: [
      ...
      "This is the same as the previous example, but will now output the double value."
    ]
  }
];

The assignment statement is an **instruction** that stores a value in a [variable](/book/part-1-instructions/2-data/5-reference/10-variable/). You use this instruction any time you want to update the value of a variable.
...
The assignment statement performs two actions. First, it calculates the value of the [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression/) (calculation) on the right-hand side of the assignment operator (the `=`). Once it has the value, it stores the value (assigns it) to the variable on the left-hand side of the assignment operator.

## Assignment Statement -- when, why, and how

You will have created a variable as you have identified a piece of information that you want to be able to change as your program runs.
Whenever you have a new value, you update the variable by using an **assignment statement**.
...
This could be a [literal](/book/part-1-instructions/1-sequence/5-reference/05-literal), a [function call](/book/part-1-instructions/2-data/5-reference/30-function), or any other [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression).
...
We do not know the value of this at the start of the program, so it is something that will change.
To manage this, you need a variable to store the value.
You might decide to call this variable `name`.
Then, you can use an assignment statement to read a response from the user and store it in the variable.
In this case, the **right-hand side** of the assignment would be a call to `read_line`, which reads input from [standard in](/book/appendix/2-computer-use/2-trailside/09-streams/#standard-input-stdin) and returns it to you.
The **left-hand side** would be the identifier of our variable, `name`. Something like this:

```c++
// Create the variable
string name;

// Call read line and store the result in name
name = read_line();
```
...
The ordering of these actions allow you to update the value of a variable using an expression involving the variable being updated. This can be very useful. For example, you might want to update the value of a variable storing the number of steps you have taken today. For example, the following code created count and then updates its value.
...
In C++ the assignment operator is `=`, with an identifier on the left-hand side and an [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression) on the right-hand side.

The assignment operator can optionally be modified with `+`, `-`, `*`, or `/`, which are shorthand for adding to, subtracting from, multiplying, and dividing the variable identified on the left-hand side of the statement.

```c++
int count = 0;

// The following is the same as count = count + 1;
count += 1;
```
...
In this example we use `read_line` to get input from the user and store it in a `name` variable.
...
int main()
{
    ...
    count = to_integer(line);

    write_line("Count is " + to_string(count) + "");
    ...
    write_line("After count = count + 1...count is now " + to_string(count) + "");
    ...
    write_line("After count += 5...count is now " + to_string(count) + "");
    ...
    write_line("After count++ ...count is now " + to_string(count) + "");
}
```
...
The good thing is that you do not need to think overly about these smaller steps, as long as you remember that the assignment statement works by evaluating the expression on the right-hand side of the `=` and then stores the result in the variable on the left-hand side.
...
- The right-hand side of the assignment operator is an [expression](/book/part-1-instructions/1-sequence/5-reference/05-expression) calculating the value to be stored.
- The left-hand side of the assignment operator is the [variable](/book/part-1-instructions/2-data/5-reference/10-variable/) into which the value is stored.
