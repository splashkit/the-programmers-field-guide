...
In the convention we are using, variables use **snake_case** for their name. This is where the first letter is lowercase and each new word in the identifier is separated with an underscore character (`_`). For example, `bottle_volume`, `my_name`, and `this_is_a_long_name` are all in snake case.
