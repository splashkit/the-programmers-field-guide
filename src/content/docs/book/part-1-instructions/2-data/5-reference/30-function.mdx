...
- Contains instructions. Similar to a procedure, but these instructions calculate a value.
- Returns a value.
...
Asking someone to do something is like a procedure call, where you are asking them to perform an action. For example, you could ask someone to "close the door". They can then carry out this action.
...
| ------------ | ------------------------ | ----------------- |
| `read_line` | none | Reads a line of data (string) from the terminal and returns it for you to use. You can use this to get user input. |
| `to_integer` | A string | Converts the string to an integer (`int`) and returns it to you. You can use this to convert data the user entered into a number. |
...
As with procedures, the function's name should identify its purpose. This helps ensure that your code is readable.

### Type Conversion Functions

You will occasionally need to convert data between different data types. A common example of this is handling user input. When the user enters data at the terminal it will come into the program as text. So if the user enters 3 and then 2, it will come into the program as "32" -- a string. This makes sense as the user can type any characters at the terminal. But what if you want that as an integer?

To help achieve this, programming languages provide utility functions that allow you to convert between different types. For now, we can use:
