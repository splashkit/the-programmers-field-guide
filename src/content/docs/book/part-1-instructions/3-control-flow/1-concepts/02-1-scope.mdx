...
With [compound statements](/book/part-1-instructions/3-control-flow/1-concepts/02-0-compound-statement), we create blocks of instructions. These have their own start (`{`) and end (`}`), and the possibility for things to be created and used within the block. This is known as **block-level scope**.
