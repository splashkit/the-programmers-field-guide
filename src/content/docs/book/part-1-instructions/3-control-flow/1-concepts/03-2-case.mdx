...
If you want the computer to skip to the end of a case statement, you must use the [break](/book/part-1-instructions/3-control-flow/1-concepts/05-1-break) statement.
...
The case statement works in a very similar way to the if statement we worked through previously. The following code will achieve the same output as the [last example](/book/part-1-instructions/3-control-flow/1-concepts/03-1-if#multiple-if-statements-combined) from the if statement page.
