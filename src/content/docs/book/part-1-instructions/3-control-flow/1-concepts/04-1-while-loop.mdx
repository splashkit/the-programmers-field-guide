...
export const sliderImagesWhileLoop = [
  {
    ...
    tipStart: 1,
    tips: [
      "Lines 1 - 5 get the user input (in this case we'll say the user entered <code>10</code>), and store it in the variable 'target' on the stack.",
...
In C# the while loop starts with the `while` keyword followed by the condition within parenthesis. The statement that is repeated follows this, which in almost all cases will be a [compound statement](../02-0-compound-statement).
