...
Let's start by looking at the mechanism known as **branching** or **selection**. Branching allows you to get the computer to take one of a number of paths based on the value of a **condition**. The code enters, evaluates the condition, then follows one of the paths. All paths then rejoin before proceeding to the next statement. Have a look at the following diagram for a visualisation of this idea.
...
- A branch has a **condition** that is evaluated and determined which path the computer will take.
