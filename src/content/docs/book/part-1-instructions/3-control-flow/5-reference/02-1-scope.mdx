...
export const sliderImagesStack = [
  {
    ...
    tips: [
      ...
      "The write_line() statement runs and prints to the terminal."
    ]
  },
  {
  ...
  },
  {
    ...
    tips: [
      ...
      "The write_line() statement completes, and the program ends."
    ]
  },
];
...
With [compound statements](/book/part-1-instructions/3-control-flow/5-reference/02-0-compound-statement), we create blocks of instructions. These have their own start (`{`) and end (`}`), and the possibility for things to be created and used within the block. This is known as **block-level scope**.
...
{
    string name = "My Name";
    write_line("We can access " + name + " here");

    {
        // We are in a new block - one nested in the program's block
        int i = 0;
        write_line(name + " and " + to_string(i) + " are accessible here");
    }

    // We are back in the program's block - i is now "out of scope"
    write_line(name + " is accessible here - but i no longer exists");

    {
        // We are in a new block.. and can create a new i variable
        int i = 1;
        write_line(name + " and a new/different " + to_string(i) + " are accessible here");
    }
}
```
