...
Before examining the use of control flow, let's have a look at a way of visually representing the flow of instructions within code. A common way of doing this is to use a **flowchart**. A flowchart is a diagram used to represent a sequence of actions, so it's perfect for representing a sequence of instructions (i.e., some code).
...
These blocks match to the structured statements in modern programming languages: [branching](/book/part-1-instructions/3-control-flow/5-reference/03-0-branching) and [looping](/book/part-1-instructions/3-control-flow/5-reference/04-0-looping) statements.
...
The addition of [jumping](/book/part-1-instructions/3-control-flow/5-reference/05-0-jumping) statements to the language allows the blocks to have **multiple exit points**: one at the end of the block's code, another at a break or continue statement.
...
You have already seen flowcharts for two of the blocks -- the pages on [branching](/book/part-1-instructions/3-control-flow/5-reference/03-0-branching) showed selection via the [if](/book/part-1-instructions/3-control-flow/5-reference/03-1-if) and [case](/book/part-1-instructions/3-control-flow/5-reference/03-2-switch) statements, and the pages on [looping](/book/part-1-instructions/3-control-flow/5-reference/04-0-looping) showed repetition via the [while](/book/part-1-instructions/3-control-flow/5-reference/04-1-while-loop), [do while](/book/part-1-instructions/3-control-flow/5-reference/04-2-do-while), and [for](/book/part-1-instructions/3-control-flow/5-reference/04-3-for-loop) loop statements.
