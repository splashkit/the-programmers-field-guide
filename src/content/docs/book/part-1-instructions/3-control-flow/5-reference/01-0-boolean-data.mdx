...
The boolean [type](/book/part-1-instructions/1-sequence/2-trailside/06-type) is used to represent **truth**. A boolean value will either be **true** or **false**. These values are used extensively in control flow statements to determine which instruction to perform next.
...
- Boolean is an [type](/book/part-1-instructions/1-sequence/2-trailside/06-type) that has been defined within the language to represent truth values.
...
- You can use `true` and `false` as boolean [literals](/book/part-1-instructions/1-sequence/2-trailside/05-literal).
