...
Control flow statements provide structure for the instructions in your code. Each of these control flow statements can contain other statements, allowing them to be nested within each other. Making this structure visible to the reader is very important. This can be achieved by indenting code within the blocks we code within these control flow statements.

## The importance of style

While programs are written to instruct the computer, they are also designed to be readable by humans.
...
Now that you have added [blocks / compound statements](/book/part-1-instructions/3-control-flow/5-reference/02-0-compound-statement) to your tool kit, indentation is something you will need to consider.

The style we use for indentation is to indent code **four spaces** within each block.
