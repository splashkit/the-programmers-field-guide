...
As with [branching](/book/part-1-instructions/3-control-flow/2-trailside/03-0-branching), a while loop is a form of control flow because it literally controls the flow of instructions to the CPU.
...
:::caution
At some point you will end up with an unexpected infinite loop. In these cases, you need to remember the details on how to [signal](/book/part-0-getting-started/2-computer-use/2-trailside/10-signals) the operating system. You can use the ctrl-c to [kill](/book/part-0-getting-started/2-computer-use/2-trailside/10-signals#killing-a-program-ctrl-c) the program.
:::
...
In C# the while loop starts with the `while` keyword followed by the condition within parenthesis. The statement to be repeated follows this, which in almost all cases will be a [compound statement](/book/part-1-instructions/3-control-flow/2-trailside/02-0-compound-statement).
...
The while loop runs **while** `i` is less than the `target`. Within the loop, we use the `i++` [assignment statement](/book/part-1-instructions/1-sequence-and-data/2-trailside/08-assignment-statement) to increase the value of `i`. This means that the value of `i` will eventually be larger than or equal to the `target`, thereby ensuring the loop will end.
